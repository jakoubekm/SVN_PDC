...
sub connect_Oracle {
	...
	($DEBUG > 8) and write_log "SECUR>   PDCserver: $PDCserver\n";
	($DEBUG > 8) and write_log "SECUR>     PDCuser: $PDCuser\n";
...
################################################################################
# Funkce SP_INIT_INITIALIZE_END - inicializace
################################################################################
sub SP_INIT_INITIALIZE_END {
	($DEBUG > 1) and write_log "$TABS>>>>>>>>>> SUB: SP_INIT_INITIALIZE_END - START\n";
	$TABS = $TABS . "    ";

	$SQL = <<SQLEND;
BEGIN
	PCKG_INIT.SP_INIT_INITIALIZE_END(:engine_id_in, :debug_in, :exit_cd, :errmsg_out, :errcode_out, :errline_out);
END;
SQLEND
	...
	$TABS = substr($TABS, 0, -4);
	($DEBUG > 1) and write_log "$TABS<<<<<<<<<< SUB: SP_INIT_INITIALIZE_END - END\n";
	return $step;
}
