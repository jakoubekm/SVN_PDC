...
#  Version:		1.0
#-------------------------------------------------------------------------------
#  Description:		Script initialize PDC
#-------------------------------------------------------------------------------
#  Version:
#  Modified:
#  Date:
...
sub connect_Oracle {
	...
	$dbh = DBI->connect( $PDCserver, $PDCuser, $PDCpassword ) || report_error( "\n\n\n\n\n\n\n\n\n\nERROR> Couldn't connect to database:\n" . $DBI::errstr . "\n" );
	$dbh->{AutoCommit}    = 0;
	$dbh->{RaiseError}    = 1;
	$dbh->{ora_check_sql} = 0;
	$dbh->{RowCacheSize}  = 16;
...
################################################################################
# Funkce SP_INIT_INITIALIZE - inicializace
################################################################################
sub SP_INIT_INITIALIZE {
	($DEBUG > 1) and write_log "$TABS>>>>>>>>>> SUB: SP_INIT_INITIALIZE - START\n";
	$TABS = $TABS . "    ";

	$SQL = <<SQLEND;
BEGIN
	PCKG_INIT.SP_INIT_INITIALIZE(:engine_id_in, :debug_in, :exit_cd, :errmsg_out, :errcode_out, :errline_out);
END;
SQLEND
	...
	while (not $success) {
		...
		$sth = $dbh->prepare($SQL) or report_error ("\n\n\n\n\n\n\n\n\n\nERROR> Couldn't prepare statement:\n" . $dbh->errstr . "\n");
		...
		$sth->execute() or report_error ("\n\n\n\n\n\n\n\n\n\nERROR> Counldn't execute statement:\n" . $sth->errstr . "\n");;
		if ($exit_cd == 0) {
	...
	$TABS = substr($TABS, 0, -4);
	($DEBUG > 1) and write_log "$TABS<<<<<<<<<< SUB: SP_INIT_INITIALIZE - END\n";
	return $step;
}
...
$LOGFILE = File::Spec->catfile("$ENV{'PMWorkflowLogDir'}", "BinLogs", "____Init_Initialize@" . $ENGINE_ID . ".log");
